...
以动机 C–D–E–G = (60,62,64,67) 为例，8 种发展技法：

| 技法 | 做法 | 示例 |
|------|------|------|
| 原样重复 | 动机完全复奏 | @0-3 → 原样 @4-7 |
| 模进 | 整体上/下移音级 1–3 次 | 60,62,64 → 62,64,65（上行二度调内模进） |
| 倒影 | 音程方向颠倒 | 60,64,67 上行 → 60,56,53 下行（调内变体 60,57,53） |
| 逆行 | 从尾到头 | 60,62,64,65 → 65,64,62,60 |
| 拉长/缩短 | 时值 ×2（庄严高潮）/ ÷2（紧迫） | d1.0 → d2.0 或 d0.5 |
| 碎片化 | 取动机头 2–3 音加速重复 | 60,62 → 60,62,60,62,60,62，副歌前爬升 |
| 音程扩展/收缩 | 首音固定，后续音拉开/收拢 | (60,62)→(60,64)→(60,67)→(60,64)→(60,62) |
| 节奏变奏 | 音序不变只改节奏 | 成本最低的发展手段 |

### 旋律统计规律（研究实证）

- **级进占比 ≈70%**：约 7 成音程为大二度以内级进，跳进 ≤30%，旋律才「可唱」
- **大跳反向解决**：≥4 半音大跳后约 70% 情况反向级进回折——`60→67→65→64`
- **轮廓**：拱形（最高点约在句长 2/3 处）；上行=振奋；下行=平静；波浪=起伏循环

### 乐句结构

- **呼应 Call & Response**：呼句结束于开放音，应句结束于主音；蓝调 AAB = 呼句重复两次 + 应答句
- **Period 前句后句**（4+4 小节）：前句半终止（停在 V），后句相同材料、完全终止（V–I）
- **Sentence 乐句式**（2+2+4）：2 小节动机 + 2 小节重复/移位 + 4 小节碎片化加速与终止式

### 律动与人性化（0.25 网格表达）

- **Swing**：三连音 0.667/0.333 无法上网格 → 用 **0.75+0.25** 近似硬 swing/shuffle
- **Laid-back 近似**：正拍音力度 -10、时值 +0.25 软化起音；push 则反拍力度 +10
