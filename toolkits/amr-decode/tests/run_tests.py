...
import amr_decode  # noqa: E402

TOOL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "amr_decode.py")
...
def test_flags():
    ...
    mid = build_midi(480, _track(ev))
    r = amr_decode.decode_midi(mid, include_events=False)
    check("--no-events 语义", "events" not in r["tracks"][0] and len(r["notes"]) == 1, str(r["tracks"][0]))
    r2 = amr_decode.decode_midi(mid, include_notes=False)
